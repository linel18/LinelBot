...
consulta.execute(CHECK)
fila_ = consulta.fetchone()

if fila_ == None:
        print("No hay un owner definido")
        user = raw_input("Inserte el user: ")
        password = input("inserte la contraseña: ")
        dates = (user, password)
        insert = "INSERT INTO owner (user, password) VALUES (?, ?)"
        arg = (user, datetime.date.today())
        insert2 = "INSERT INTO test (user, fecha) VALUES (?, ?)"
        consulta.execute(insert, dates)
        consulta.execute(insert2, arg)
consulta.close()
conexion.commit()
conexion.close()
...
def conectar():
        ...
        while connection == False:
                print("Conectando a %s:%s" % (HOST, PORT))
                try:
                        global s
                        s=socket.socket()
...
def drop_permisos_op(user):
        ...
        try:
                ...
                if consulta.execute(sql2):
                       filas1 = consulta.fetchone()
                       if "op-voice" in filas1[2] and filas1[1] == CHAN_A:
                               ...
                               sql5 = "DELETE FROM permisos WHERE id = %s" % filas[0]
                               consulta.execute(sql5)
                               s.send("PRIVMSG %s :Permisos removidos\n" % CHAN_A)
...
while 1:
        try:
                ...
                for line in temp:
                        ...
                        try:
                                hostuser = line2[0].split()[0].split(":")[1]
                        except: 
                                hostuser = None
                        n = ''
                        try:
                        ...
                        def check_user(user, CHAN_A=CHAN_A):
                                conexion = sqlite3.connect('database.sql')
                                consulta = conexion.cursor()
                                user = user_account(user)
                                sql1 = "SELECT * FROM test WHERE user = \"%s\"" % user
                        ...
                        #        s.send("MODE %s +b %s\n" %s (chan, user))
                        if line2[0] in "ERROR :Closing link:":
                                s.close()
                                conectar()      
                        if len(line2) > 1:

                                if line2[1].startswith("*reload"):
                                        connection = False
                                        s.send("QUIT :Reloading...\n")
                                        conectar()
                                ...
                                if line2[1].startswith("*add op"):
                                        ...
                                        if not hostuser in hostmask:
                                                send_msg("Permisos Insuficientes.")
                                        
                                        elif user == None:
                                                send_msg("Error: no se a pasado ningún usuario.")
                                        elif hostuser in hostmask:
                                ...
                                if line2[1] == "*quit":
                                        if check_user(username):
                                                s.send("QUIT LinelBot\n")
                                                sys.exit() 
                                        if not check_user(username):
                                                send_msg("\x05Permisos insuficientes\x05")
                                ...
                                if len(linea_) > 1:
                                        ...
                                        if linea_[1].startswith("auth"):
                                                ...
                                                try:
                                                        ...
                                                        passw = None
                                                if user == None:
                                                        send_priv(username, "Error: no se han pasado parametros")
                                                if passw == None:
                                                        send_priv(username, "Error: no se han pasado parametros")
                                                if user == FOUNDER and str(PASS) == passw:
                                                        
                                                        raw = linea_[0].split()
                                                        mask = raw[0].split(":")
                                                        hostmask.append(mask[1])
                                                        send_priv(username, "Se ha logueado correctamente.")
                                                else:
                                                        send_priv(username, "Error: Datos inválidos.")
                                                consulta.close()
                                                connection.close()
                        ...
                        if line[0].find("NOTICE " + NICK):
                                ...
                                if len(lineal) > 1:
                                        if lineal[1].startswith("\x01PING"):
                                                ...
                                                send_priv(pings[username][0], username + " lag : "+ str(round(lag, 2)) + "s")
        except KeyboardInterrupt:
                s.send("QUIT :KeyboarInterrupt\n")
                s.close()
                sys.exit()
